...
uses
  // VCL
  Messages, Windows, SysUtils, Classes, Graphics, Menus, Controls, Forms,
...
type
  ...
  private
    procedure SelectCells(CellHit: TGridCoord);
  private
    FAnchor: TGridCoord;
...
procedure TCustomGrid.SelectCells(CellHit: TGridCoord);
  ...
  procedure SelectRow(Value: Integer);
  var
    GridRect: TGridRect;
  begin
    GridRect.Left := 1;
...
begin
  if (CellHit.X = 0)and(CellHit.Y = 0) then SelectAll
  else if CellHit.X = 0 then SelectRow(CellHit.Y) else
  if CellHit.Y = 0 then SelectCol(CellHit.X);
end;
